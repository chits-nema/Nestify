// ------------- LOCATION SELECTION -------------
// Make startSwipeWithLocation accessible globally (defined in tinder.js)
window.startSwipeWithLocation = function() {
  if (typeof startSwipeWithLocation !== 'undefined') {
    startSwipeWithLocation();
  } else {
    console.error('tinder.js not loaded yet');
  }
};

// ------------- DUMMY HOMES FOR FILTER/LISTINGS VIEW -------------
// (This can later be connected to the real backend exactly like tinder.js)
const homes = [
  {
    id: 1,
...
function goToSwipe() {
  const section = document.getElementById("tinder");
  if (section) {
    section.scrollIntoView({ behavior: "smooth" });
  }
}
...
function displayHomes(homesToDisplay) {
  ...
  if (!homesToDisplay || homesToDisplay.length === 0) {
  ...
  card.innerHTML = `
    ...
    <div class="listing-content">
      ...
      <button class="favorite-btn ${isFavorite ? "active" : ""}"
              onclick="toggleFavorite(event, ${home.id})">
        ${isFavorite ? "❤️" : "🤍"}
      </button>
    </div>
  `;
...
function searchHomes() {
  ...
  const filtered = homes.filter((home) => {
    return (
      home.title.toLowerCase().includes(searchInputValue) ||
      home.location.toLowerCase().includes(searchInputValue) ||
      home.type.toLowerCase().includes(searchInputValue)
    );
  });
...
function applyFilters() {
  ...
  if (bedroomFilter !== "all") {
    filtered = filtered.filter(
      (home) => home.bedrooms >= parseInt(bedroomFilter)
    );
  }

  if (priceFilter !== "all") {
    filtered = filtered.filter(
      (home) => home.price <= parseInt(priceFilter)
    );
  }
